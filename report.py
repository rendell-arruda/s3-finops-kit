...
def get_bucket_region(s3_client, bucket_name):
    ...
    # us-east-1 retorna None na API da AWS, por isso o tratamento abaixo
...
def collect():
    ...
    for bucket in buckets:
        ...
        # descobre a região real do bucket
        regiao = get_bucket_region(s3, nome)

        # ignora buckets fora das regiões que queremos analisar
...
def salvar_csv(dados):
    ...
    print(f"\ndata.csv salvo com {len(dados)} registros.")


if __name__ == "__main__":
    print("Coletando dados dos buckets S3...\n")
    dados = collect()
    salvar_csv(dados)
